...
Card Player::discard(int cardIndex, std::string list_of_cards) {
    // SI c'est une défausse classique -> depuis la main
    ...
    // SI c'est une défausse de carte permanente -> suite à une carte action
    else {
        if (cardIndex < 0 || cardIndex >= this->hand.size()) {
            throw std::out_of_range("Card index out of range");
        }
...
void Player::play_permanent_card(int cardIndex)
{
    ...
    case CardName::ARMURE:
        this->additional_difficulty += 1;
        std::cout << "Votre armure augmente de 1, vous avec maintenant " << this->get_additional_difficulty() << " d'armure supplémentaire." << std::endl;
        break;
    case CardName::CONCENTRATION:
        this->additional_weapon += 1;
        std::cout << "Votre nombre d'armes jouables pendant le même tour augmente de 1, vous pouvez maintenant jouer " << this->get_additional_weapon() << " armes(s) supplémentaire." << std::endl;
        break;
    case CardName::ATTAQUE_RAPIDE:
        this->additional_damage += 1;
        std::cout << "Vos dégats d'armes bonus augmentent 1, elles inflignent maintenant " << this->get_additional_damage() << " dégats supplémentaire." << std::endl;
        break;
    default:
...
void Player::discard_permanent_card(int cardIndex) 
{
    ...
    discard(cardIndex, "permanent_card_played"); // On défausse depuis les cartes permanentes déjà jouées
}
...
int Player::play_card_phase() {
    print_hand();
    std::cout << "Entrez le numéro d'une carte pour la jouer, ou -1 si vous avez fini / passez votre tour : " << std::endl;
    int response;
    std::cin >> response;
    if (response == -1) {
		return -1;
    ...
    system("cls");
    std::cout << "Vous avez précédemment joué : " << hand[response].print_name() << std::endl;
    std::cout << std::endl;
    return response;
}
...
std::vector<Card> Player::discard_phase() {
    ...
    while (hand.size() > 7) {
        ...
        std::cout << "Votre main est pleine veuillez défausser une carte." << std::endl;
        print_hand();
    ...
    if (discarded_cards_vector.size() > 0) { // Si des cartes ont étés défaussées on affiche le résumé
        system("cls");
        std::cout << "Vous avez défaussé : " << std::endl;
...
int Player::get_attacked(Player& player, Card card) {
    // On vérifie si le joueur attaqué a une parade
    auto it = std::find_if(hand.begin(), hand.end(), [](const Card& card) {
        return card.get_name() == CardName::PARADE;  
        });
    // Si c'est le cas, on demande au joueur attaqué s'il souhaite l'utiliser
    if (it != hand.end()) {
        std::cout << "---------- Joueur " << this->name << ", vos stats ----------" << std::endl;
        std::cout << "HP : " << this->HP << std::endl;
        std::cout << "Défense : " << this->damage_mitigation << std::endl;
        std::cout << "Vous avez été attaqué par " << player.get_name() << " avec la carte " << card.print_name() << ". Vous allez subir " << card.get_damage() + player.get_additional_damage() - this->get_damage_mitigation() << " dégat(s)." << std::endl;
        ...
        if (response == 1) {
			...
			discard(index);
			std::cout << "Vous avez utilisé une parade, vous n'avez pas subi de dégat" << std::endl;
			return index;
		}
        else {
			std::cout << "Vous n'avez pas utilisé de parade, vous subissez " << card.get_damage() << " dégat(s)" << std::endl;
        }
    }
    else {
        std::cout << "Joueur " << this->name << " vous n'avez pas de parade dans votre main, vous subissez " << card.get_damage() + player.get_additional_damage() - this->get_damage_mitigation() << " dégats" << std::endl;
...
int Player::select_target(int nbrPlayer) {
    std::cout << "Entrez le numéro du joueur que vous souhaitez cibler : " << std::endl;
    int response;
    std::cin >> response;
...
void Player::get_Jujitsued() {
    ...
    std::cout << " vous avez été affecté par la carte Jujitsu" << std::endl; // le formatage étrange de la ligne est volontaire
    ...
    if (weapon_cards_in_hand.size() > 0) {
        ...
        std::cout << "Vous devez en défausser une ou perdre un point de vie." << std::endl;
        std::cout << "Que souhaitez vous faire ? (entrez un index pour défausser la carte en question, ou -1 pour perdre un point de vie) : " << std::endl;
        int response;
        std::cin >> response;
        if (response == -1) {
            this->HP -= 1;
        ...
        while (std::cin.fail() || response < -1 || response > hand.size() - 1) {
            std::cout << "Réponse invalide" << std::endl;
            std::cin.clear();
            std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n'); // ignore le reste de la ligne
            std::cout << "Que souhaitez vous faire ? (entrez un index pour défausser la carte en question, ou -1 pour perdre un point de vie) : " << std::endl;
            std::cin >> response;
    ...
    // SI le joueur n'a pas de cartes arme en main
        ...
        std::cout << "Vous ne possedez pas de cartes armes, par conséquent vous êtes forcé de perdre un point de vie" << std::endl;
        this->HP -= 1;
...
int Player::select_geihsha_mode(Player& target) {
    ...
    if (target.get_permanent_card_played().size() > 0) {
        ...
        std::cout << "Souhaitez vous défausser une carte permanente déjà jouée (1) ou une carte aléatoire de sa main (2) ? :" << std::endl;
        std::cin >> response;
        ...
        std::cout << "Le joueur" << target.get_name() << "n'a pas encore joué de cartes permanentes" << std::endl;
        std::cout << "Vous devez donc défausser une carte aléatoire de sa main" << std::endl;
		return 2;
    }
}

void Player::get_geishaed(int geishaMode, int cardIndexToDiscard) {
    std::cout << " vous avez été affecté par la carte Geisha" << std::endl; // le formatage étrange de la ligne est volontaire
    if (geishaMode == 1) {
        std::cout << "Cette carte permanente a été défaussée : " << this->permanent_card_played[cardIndexToDiscard].print_name() << std::endl;
        ...
        int random_index = distrib(gen);
		std::cout << "Cette carte de la main a été défaussée : " << this->hand[random_index].print_name() << std::endl;
...
void Player::get_Cri_de_guerred() {
    ...
    std::cout << " vous avez été affecté par la carte Cri de guerre" << std::endl; // le formatage étrange de la ligne est volontaire
    // SI le joueur a des cartes armes en main
    if (parade_in_hand.size() > 0) {
        ...
        std::cout << "Vous devez en défausser une ou perdre un point de vie." << std::endl;
        std::cout << "Que souhaitez vous faire ? (entrez un 1 pour défausser une parade, ou 0 pour perdre un point de vie) : " << std::endl;
        int response;
        std::cin >> response;
        while (std::cin.fail() || response < 0 || response > 1) {
            std::cout << "Réponse invalide" << std::endl;
        ...
        if (response == 0) {
            ...
            // On trouve la première parade dans la main du joueur et on la défausse
            auto it = std::find_if(hand.begin(), hand.end(), [](const Card& card) {
                return card.get_name() == CardName::PARADE;
				});
            discard(it - this->hand.begin());
...
void Player::game_phase()
{
    ...
    while (play && HP > 0) {
        ...
        print_hand();
        std::cout << "Choisissez une carte à jouer (entrez l'index de la carte) :";
        int cardIndex;
        std::cin >> cardIndex;
        ...
        if (card.is_permanent_card()) {
            ...
            // TODO faut implémenter la fonction attack

        }
        HP -= card.get_HP();
        hand.erase(hand.begin() + cardIndex);
        std::cout << "Carte jouée " << card.print_name() << std::endl;
    }
    if (HP <= 0) {
