...
void Deck::initActionCards() {
    for (int i = 0; i < 3; ++i) {
        ...
        ActionCard meditation = actionCardsVector[5]; // Méditation
        cards.push_back(meditation);
    }
    for (int i = 0; i < 4; ++i) {
        ActionCard cri_de_guerre = actionCardsVector[1]; // Cri de guerre
        cards.push_back(cri_de_guerre);
        ActionCard ceremonie_du_the = actionCardsVector[0]; // Cérémonie du thé
...
void Deck::initPermanentCards() {
    for (int i = 0; i < 2; ++i) {
        PermanentCard codeDuBushido = permanentCardsVector[0]; // Code du Bushido
        cards.push_back(codeDuBushido);
    }
    for (int i = 0; i < 3; ++i) {
        ActionCard attaqueRapide = actionCardsVector[3]; // Attaque Rapide
        cards.push_back(attaqueRapide);
    }
    for (int i = 0; i < 4; ++i) {
        ActionCard armure = actionCardsVector[0]; // Armure
        cards.push_back(armure);
    }
    for (int i = 0; i < 6; ++i) {
        ActionCard concentration = actionCardsVector[4]; // Concentration
